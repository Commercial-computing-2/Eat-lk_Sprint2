				...
				if (isset($usr_id)) {
					...
					if (mysqli_num_rows($result) > 0) {
						...
						while ($row = mysqli_fetch_assoc($result)) {
							...
							echo '<div class="col-2">LKR ' . number_format((float)($row_data['ctlog_prc']), 2, '.', '') . '</div>';
						...
						if ($payment_type == 'paypal') {
							echo '<h4 class="col text-center text-primary">LKR ' . number_format((float)(round($tot_prc + $tot_svc, 1)), 2, '.', '') . ' (Paid)</h4>';
							echo '</div>';

							echo '<div class="row">';
							echo '<p class="col text-center ">Pay by Paypal. </p>';
						} else {
							echo '<h4 class="col text-center text-success">LKR ' . number_format((float)(round($tot_prc + $tot_svc, 1)), 2, '.', '') . ' (Unpaid)</h4>';
							echo '</div>';
